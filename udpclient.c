...
/* modified Emily Evans and Madison Adams */
/* November 16, 2019 */
...
struct timeval timeout; /* Struct used to set the timeout value */
...
int timeoutExpCounter = 0; //How many times the timeout expired
...
int main(int argc, char* argv[]) {
    ...
    if(argc == 3){
        ...
        printf("Format to type:\n./udpclient <timeout exponent> <input file>\n");
        exit(0);
    }
    
    int temp_usec = pow(10.0,timeoutExp); //This is the timeout value in microseconds only
    timeout.tv_sec = temp_usec * 1/(double)pow(10.0,6.0); //Set the timeout value in seconds
    timeout.tv_usec = temp_usec%1000000; //Sets the remainder of microseconds to the microsecond field.
    ...
    setsockopt(sock_client, SOL_SOCKET, SO_RCVTIMEO, (const char*)&timeout, sizeof(timeout)); //Sets the timeout value for the socket defined
    ...
    exit(1);
        ...
        while (fgets(str, 80, fp) != NULL) {
            ClearData(dataSend.payload); //Clear the data field of the struct
            dataSend.count = 0; //Set the count to 0
            
            /* removing null character */
            i = 0;
            while(str[i] != '\0'){
                dataSend.payload[i] = str[i];
                dataSend.count++;
                i++;
            }
        ...
        printf("End of File Reached, EOF Packet sent with Sequence number %i, printing statistics:\n\n", ntohs(dataSend.seqNum));
	printf("Number of data packets transmitted(initial transmission only): %i\n", initialPackets);
	printf("Total number of data bytes transmitted: %i\n", totalBytesTransmitted);
	printf("Total number of retransmissions: %i\n", retransmittedPackets);
	printf("Total number of data packets transmitted: %i\n", totalPackets);
	printf("Number of ACKs received: %i\n", ACKsReceived);
	printf("How many times the timeout expired: %i\n", timeoutExpCounter);
